  ...
  <el-row style="margin-top: 50px;">
    <el-col :span="8" :offset="8">
      <el-card>
        ...
        <el-row>
          <el-col :span="24">
            <el-input ref="user" class="login_input" v-model="username" placeholder="邮箱/手机号/账号">
            </el-input>
          </el-col>
          <el-col :span="24">
...
console.log('登陆成功')
            var date = new Date();
            date.setDate(date.getDate()+7);//设置过期时间
            //第一个值 设置的cookie的name
            //第二个值 数据库中的登陆字符串
            //第三个值 过期时间
            //set     cnode的挂载cookie写法
            this.$cookie.set('token_id',this.accessToken,{expires: date.toGMTString()});

            this.editLogined(true); //是否登陆的判断状态

            this.getUserInfos();     //个人信息，个人资料
